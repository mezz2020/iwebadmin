...
<script>
import { getProductInfo, createProduct, getTypeList, getExtraList, getUploadUrl } from '@/api/product'
export default {
  name: 'product_add',
  data () {
    return {
  ...
  mounted () {
    ...
    this.loadProductInfo(product_id)
  },
  methods: {
    loadProductInfo (product_id = 0) {
      const that = this
      let param = { product_id: product_id }
      getProductInfo(param).then((res) => {
        let data = res.data
        if (data.code === 0) {
          let product_info = data.result
          product_info.best = product_info.is_best === 1
          product_info.is_onsale = product_info.onsale === 1
          that.formData = product_info

          // 处理商品相册
          let product_photos = product_info.photos
          product_photos.map((val, idx) => {
            that.product_imgs.uploadList.push({
              name: 'product_image_' + val.photo_id,
              url: val.url,
              showProgress: false,
              status: 'finished'
            })
          ...
          // 处理商品属性
          let product_attrs = data.extra
          let input_attrs = []
          getExtraList({ type_id: product_info.type_id }).then(res => {
            const data = res.data
            if (data.code === 0) {
              let attr_list = data.result
              if (product_attrs) {
                attr_list.map((val, index) => {
                  let attr_id = val.attr_id
                  let attr_values = []
                  product_attrs[attr_id].map((sval, sidx) => {
                    attr_values.push({
                      'value': sval.attr_value,
                      'price': sval.price,
                      'attr_img': sval.attr_image
                    })
                  })
                  input_attrs.push({ 'attr_id': attr_id, 'attr_values': attr_values })
                })
              }
